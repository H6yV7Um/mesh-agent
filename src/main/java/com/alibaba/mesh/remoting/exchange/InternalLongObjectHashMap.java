 ...
 */
public class InternalLongObjectHashMap<V> implements LongObjectMap<V> {
    ...
    private final Set<Long> keySet = new InternalLongObjectHashMap.KeySet();
    private final Set<Entry<Long, V>> entrySet = new InternalLongObjectHashMap.EntrySet();
    private final Iterable<PrimitiveEntry<V>> entries = new Iterable<PrimitiveEntry<V>>() {
        @Override
        public Iterator<PrimitiveEntry<V>> iterator() {
            return new InternalLongObjectHashMap.PrimitiveIterator();
        }
    };
    /**
    ...
    public InternalLongObjectHashMap() {
        this(DEFAULT_CAPACITY, DEFAULT_LOAD_FACTOR);
    }

    public InternalLongObjectHashMap(int initialCapacity) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR);
    }

    public InternalLongObjectHashMap(int initialCapacity, float loadFactor) {
    ...
    public void putAll(Map<? extends Long, ? extends V> sourceMap) {
        if (sourceMap instanceof InternalLongObjectHashMap) {
            // Optimization - iterate through the arrays.
            @SuppressWarnings("unchecked")
            InternalLongObjectHashMap<V> source = (InternalLongObjectHashMap<V>) sourceMap;
    ...
    private boolean removeAt(final int index) {
        ...
        for (V value = values[i]; value != null; value = values[i = probeNext(i)]) {
            long key = keys[i];
            int next = hashIndex(key);
            if (i < next && (next <= nextFree || nextFree <= i) ||
                    next <= nextFree && nextFree <= i) {
    ...
    private void removeAt0(final int index) {
        --size;
        // Clearing the key is not strictly necessary (for GC like in a regular collection),
        // but recommended for security. The memory location is still fresh in the cache anyway.
        keys[index] = 0;
    ...
    private final class EntrySet extends AbstractSet<Entry<Long, V>> {
        ...
        public Iterator<Entry<Long, V>> iterator() {
            return new InternalLongObjectHashMap.MapIterator();
        }

        @Override
        public int size() {
            return InternalLongObjectHashMap.this.size();
        }
    }
    ...
    private final class KeySet extends AbstractSet<Long> {
        ...
        public boolean contains(Object o) {
            return InternalLongObjectHashMap.this.containsKey(o);
        }

        @Override
        public boolean remove(Object o) {
            return InternalLongObjectHashMap.this.remove(o) != null;
        }
        ...
        @Override
        public void clear() {
            InternalLongObjectHashMap.this.clear();
        }
    ...
    private final class MapIterator implements Iterator<Entry<Long, V>> {
        private final InternalLongObjectHashMap.PrimitiveIterator iter = new InternalLongObjectHashMap.PrimitiveIterator();
        ...
        public Entry<Long, V> next() {
            ...
            return new InternalLongObjectHashMap.MapEntry(iter.entryIndex);
        }
